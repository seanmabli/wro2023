...
def main():
  ...
  lineFollowingDistance(distance=270, sensor=LeftColor, sideofsensor='out', speed=300, proportion=0.8)
  ...
  # ** Container Pickup (Without Boat Dropoff) ** 
  position = calibratePos(50)
  position += straight(containerPositions[2] - position, deceleration=True)
  armGrab("up->down")
  ...
  position += straight(containerPositions[0] - position, deceleration=True)
  armGrab("up->down", speed=greenArmDownSpeed)
  armGrab("down->midup", speed=greenArmUpSpeed)
  armGrab("midup->up")
  time.sleep(0.3)

  # ** Return to Intersection B **
  durn(turn=-165, type="tank", speed=300)
  sweep(sensor=LeftColor, direction="left", whiteFirst=True)
  lineFollowingBlack(sensor=LeftColor, sideofsensor='out', blackthreshold=10, whitethreshold=45, speed=200)
  straight(250)
