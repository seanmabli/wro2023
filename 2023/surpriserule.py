...
def main():
  ...
  lineFollowingDistance(distance=105, sensor=LeftColor, sideofsensor='in', speed=400)
  ...
  # ** Pickup Blacks **
  durn(turn=-340, type="tank")
  bigArmGrab(movement="open")
  straight(-300)
  bigArmGrab(movement="close")

  # ** Back to Intersection B ** 
  sweep(sensor=LeftColor, direction="left", whiteFirst=True)
  lineFollowingBlack(sensor=LeftColor, sideofsensor='out', blackthreshold=10, whitethreshold=45, speed=300, proportion=0.8)
  straight(135)
  durn(turn=155, type="tank", speed=150)
  time.sleep(2)
  sweep(sensor=LeftColor, direction="left", whiteFirst=True, threshold=(0, 10), speed=100)
  
  # ** Intersection B (from Intersection A) to Intersection C (No Sweep) **
  lineFollowingDistance(distance=250, sensor=LeftColor, sideofsensor='out', speed=200, proportion=1)
  ...
  sweep(sensor=LeftColor, direction="left", whiteFirst=True)
  lineFollowingDistance(distance=200, sensor=LeftColor, sideofsensor='out', speed=300, proportion=0.8)
  durn(turn=-300, type="tank", speed=300)
  straight(-330)
  bigArmGrab(movement="open")
  ...
  lineFollowingDistance(distance=140, sensor=LeftColor, sideofsensor='out', speed=200, proportion=0.8)
  lineFollowingBlack(sensor=LeftColor, sideofsensor='out', blackthreshold=15, whitethreshold=45, speed=200)
  liftGrab(movement="down")

  # ** Intersection D (from Open Ocean Base, No Sweep) to Crane B **
  lineFollowingDistance(distance=460, sensor=LeftColor, sideofsensor='out', speed=200)
  sturn(rl="left", fb="forward", turn=-50)
  straight(-200)
  liftGrab(movement="up")

  # ** Return to Intersection D **
  durn(turn=-300, type="tank", speed=300)
  straight(200)
  ...
  sweep(sensor=LeftColor, direction="left", whiteFirst=True, threshold=(0, 10))
  lineFollowingBlack(sensor=LeftColor, sideofsensor='out', blackthreshold=10, whitethreshold=45, speed=300, proportion=1)

  # Intersection C to Crane A **
  lineFollowingDistance(distance=120, sensor=LeftColor, sideofsensor='out', speed=200)
  durn(turn=150, type="tank")
  straight(-600)
  ...
  lineFollowingDistance(distance=140, sensor=LeftColor, sideofsensor='out', speed=200, proportion=0.8)
  lineFollowingBlack(sensor=LeftColor, sideofsensor='out', blackthreshold=15, whitethreshold=45, speed=200)
  liftGrab(movement="down", percentage=1.15)

  lineFollowingDistance(distance=540, sensor=LeftColor, sideofsensor='out', speed=200)
  sturn(rl="left", fb="forward", turn=-50)
  straight(-200)
  liftGrab(movement="up")
  straight(-400)
...
def liftGrab(movement="down", percentage=1, hold=False, stop=False, speed=400):
  if movement == "down":
    ArmMotor.stop()
    ArmMotor.run_angle(-speed, 120 * percentage)
    time.sleep(0.01)
  elif movement == "up":
    ArmMotor.run(speed)
